...
| `pageSize` | number | No | Number of templates to return per page \(default: 20, max: 200\). When paginating with pageToken, pass the same pageSize used on the first request to keep page boundaries consistent. |
| `pageToken` | string | No | Page token from a previous response \(nextPageToken\) to fetch the next page |
