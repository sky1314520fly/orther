...
| --------- | ---- | ----------- |
| `status` | number | HTTP status code returned by SAP |
| `data` | json | Array of A_Customer entities, or `\{ results, __count?, __next? \}` when pagination metadata is present \(the integration unwraps the OData v2 `d` envelope\). Properties below describe each customer item. |
