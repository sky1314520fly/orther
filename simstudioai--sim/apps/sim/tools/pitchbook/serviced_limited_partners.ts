...
export const pitchbookServicedLimitedPartnersTool: ToolConfig<
  PitchbookProfileParams,
  PitchbookResponse
> = {
  id: 'pitchbook_serviced_limited_partners',
  name: 'PitchBook Serviced Limited Partners',
  description: 'Retrieve the limited partners a service provider currently and formerly serves',
  version: '1.0.0',
  ...
  request: {
    url: (params) =>
      `${PITCHBOOK_API_BASE}/service-providers/${params.pbId.trim()}/serviced-limited-partners`,
    method: 'GET',
    headers: (params) => pitchbookAuthHeaders(params.apiKey, params.currency),
  },

  transformResponse: async (response: Response) => {
    await throwIfNotOk(response, 'Failed to fetch serviced limited partners')
