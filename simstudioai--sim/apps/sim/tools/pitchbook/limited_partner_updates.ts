...
export const pitchbookLimitedPartnerUpdatesTool: ToolConfig<
  PitchbookUpdatesParams,
  PitchbookResponse
> = {
  id: 'pitchbook_limited_partner_updates',
  name: 'PitchBook Limited Partner Updates',
  description:
  ...
  request: {
    url: (params) => {
      ...
      return `${PITCHBOOK_API_BASE}/limited-partners/${params.pbId.trim()}/updates${query ? `?${query}` : ''}`
    },
    method: 'GET',
