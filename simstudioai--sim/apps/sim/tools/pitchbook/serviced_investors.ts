...
export const pitchbookServicedInvestorsTool: ToolConfig<PitchbookProfileParams, PitchbookResponse> =
  {
    id: 'pitchbook_serviced_investors',
    name: 'PitchBook Serviced Investors',
    description: 'Retrieve the investors a service provider currently and formerly serves',
    version: '1.0.0',
    ...
    request: {
      url: (params) =>
        `${PITCHBOOK_API_BASE}/service-providers/${params.pbId.trim()}/serviced-investors`,
      method: 'GET',
      headers: (params) => pitchbookAuthHeaders(params.apiKey, params.currency),
    },

    transformResponse: async (response: Response) => {
      await throwIfNotOk(response, 'Failed to fetch serviced investors')
