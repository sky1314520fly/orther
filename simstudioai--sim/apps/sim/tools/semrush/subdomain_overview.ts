import type {
  SemrushSubdomainOverviewParams,
  SemrushSubdomainOverviewResponse,
  SemrushSubdomainOverviewRow,
...
export const semrushSubdomainOverviewTool: ToolConfig<
  SemrushSubdomainOverviewParams,
  SemrushSubdomainOverviewResponse
> = {
  id: 'semrush_subdomain_overview',
  name: 'Semrush Subdomain Overview',
  ...
  transformResponse: async (response: Response) => {
    const rows = await readSemrushReport<SemrushSubdomainOverviewRow>(response, COLUMNS)
  ...
  outputs: {
    overview: {
      type: 'json',
      description: 'Organic and paid search overview for the subdomain',
      nullable: true,
      properties: {
        domain: {
          type: 'string',
