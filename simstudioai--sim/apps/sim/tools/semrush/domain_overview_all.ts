import type {
  SemrushDomainOverviewAllParams,
  SemrushDomainOverviewAllResponse,
  SemrushDomainOverviewAllRow,
...
export const semrushDomainOverviewAllTool: ToolConfig<
  SemrushDomainOverviewAllParams,
  SemrushDomainOverviewAllResponse
> = {
  id: 'semrush_domain_overview_all',
  name: 'Semrush Domain Overview (All Databases)',
  description:
  ...
  params: {
    ...
    displaySort: {
      ...
      required: false,
      visibility: 'user-or-llm',
      description: 'Sort order, for example rk_asc, or_desc, or ot_desc',
    },
  },
  ...
  transformResponse: async (response: Response) => {
    const rows = await readSemrushReport<SemrushDomainOverviewAllRow>(response, COLUMNS)
  ...
  outputs: {
    databases: {
      type: 'array',
      description: 'One row of domain totals per regional database',
      items: {
        type: 'object',
        properties: {
          database: {
