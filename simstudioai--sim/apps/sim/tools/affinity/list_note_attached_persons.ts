...
import { PERSON_REFERENCE_OUTPUT_PROPERTIES } from '@/tools/affinity/types'
import {
  affinityHeaders,
  buildAffinityUrl,
  requireId,
...
export const affinityListNoteAttachedPersonsTool: ToolConfig<
  AffinityNoteCollectionParams,
  AffinityCollectionResponse<'persons'>
> = {
  id: 'affinity_list_note_attached_persons',
  name: 'Affinity List Note Attached Persons',
  ...
  request: {
    url: (params) =>
      buildAffinityUrl(
        `/notes/${encodeURIComponent(requireId(params.noteId, 'noteId'))}/attached-persons`,
  ...
  outputs: {
    persons: {
      type: 'array',
      description: 'Persons attached to the note',
      items: { type: 'object', properties: PERSON_REFERENCE_OUTPUT_PROPERTIES },
    },
