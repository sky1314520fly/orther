import type {
  ...
  AffinitySearchListEntriesParams,
...
export const affinitySearchListEntriesTool: ToolConfig<
  AffinitySearchListEntriesParams,
  AffinityCollectionResponse<'listEntries'>
> = {
  id: 'affinity_search_list_entries',
  name: 'Affinity Search List Entries',
  description:
  ...
  params: {
    ...
    listId: {
      ...
      description: 'The list ID to search',
    },
    filters: {
      type: 'json',
      required: false,
  ...
  request: {
    url: (params) =>
      buildAffinityUrl(
        `/lists/${encodeURIComponent(requireId(params.listId, 'listId'))}/list-entries/search`,
        {
  ...
  outputs: {
    listEntries: {
      type: 'array',
      description: 'Matching rows on the list',
