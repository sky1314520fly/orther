...
import { LIST_OUTPUT_PROPERTIES } from '@/tools/affinity/types'
import {
...
interface AffinityListEntityListsParams extends AffinityCursorParams {
  entityType: string
  entityId: string
}

export const affinityListEntityListsTool: ToolConfig<
  AffinityListEntityListsParams,
  AffinityCollectionResponse<'lists'>
> = {
  id: 'affinity_list_entity_lists',
  name: 'Affinity List Entity Lists',
  ...
  params: {
    ...
    entityType: {
      ...
      description: 'Which entity to look up the lists of: companies or persons',
    },
    entityId: {
      type: 'string',
      required: true,
  ...
  request: {
    url: (params) => {
      ...
      return buildAffinityUrl(`/${entityType}/${entityId}/lists`, {
  ...
  outputs: {
    lists: {
      type: 'array',
      description: 'Lists the entity appears on',
      items: { type: 'object', properties: LIST_OUTPUT_PROPERTIES },
    },
