...
export const affinityListNoteAttachedOpportunitiesTool: ToolConfig<
  AffinityNoteCollectionParams,
  AffinityCollectionResponse<'opportunities'>
> = {
  id: 'affinity_list_note_attached_opportunities',
  name: 'Affinity List Note Attached Opportunities',
  ...
  request: {
    url: (params) =>
      buildAffinityUrl(
        `/notes/${encodeURIComponent(requireId(params.noteId, 'noteId'))}/attached-opportunities`,
  ...
  outputs: {
    opportunities: {
      type: 'array',
      description: 'Opportunities attached to the note',
