...
export const tableGetSchemaTool: InternalToolConfig<TableGetSchemaParams, TableGetSchemaResponse> =
  {
    id: 'table_get_schema',
    name: 'Get Schema',
    ...
    operation: {
      input: (params: TableGetSchemaParams) => {
        const workspaceId = params._context?.workspaceId
        if (!workspaceId) {
          throw new Error('Workspace ID is required in execution context')
        }

        return { tableId: params.tableId, workspaceId }
      },
    },
