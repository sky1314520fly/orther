...
export const updateTool: InternalToolConfig<MSSQLUpdateParams, MSSQLUpdateResponse> = {
  ...
  params: {
    ...
    data: {
      ...
      required: true,
      visibility: 'user-or-llm',
      description: 'Data object with the columns to set (key-value pairs)',
    },
    where: {
      type: 'string',
      required: true,
      visibility: 'user-or-llm',
      description: 'WHERE condition identifying the rows to update',
    },
  },
  ...
  transformResponse: async (response: Response) => {
    ...
    if (!response.ok) {
      throw new Error(data.error || 'Microsoft SQL Server update failed')
    }
