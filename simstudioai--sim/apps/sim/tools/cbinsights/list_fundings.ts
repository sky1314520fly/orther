...
export interface CbInsightsListFundingsParams extends CbInsightsOrgListParams {
  limit?: number | string
  nextPageToken?: string
}

export const cbinsightsListFundingsTool: InternalToolConfig<
  CbInsightsListFundingsParams,
  CbInsightsListResponse
> = {
  id: 'cbinsights_list_fundings',
  name: 'CB Insights List Fundings',
  description:
  ...
  outputs: {
    orgs: {
      type: 'json',
      description:
        'Organizations as [{orgId, fundings, capTableHistory}]. An organization with no data is omitted from the response.',
    },
    nextPageToken: {
      type: 'string',
      nullable: true,
