...
export async function executeInsert(
  ...
  return executeStatement(client, resourceArn, secretArn, database, sql, parameters, signal)
}
...
export async function executeIntrospect(
  ...
  const detectedEngine =
    engine || (await detectEngine(client, resourceArn, secretArn, database, signal))

  if (detectedEngine === 'aurora-postgresql') {
    const schema = schemaName || 'public'
    return introspectPostgresql(client, resourceArn, secretArn, database, schema, signal)
  }
  const schema = schemaName || database || ''
  if (!schema) {
    throw new Error('Schema or database name is required for MySQL introspection')
  }
  return introspectMysql(client, resourceArn, secretArn, database, schema, signal)
}
