...
import { extendParseInputSchema } from '@/lib/internal/extend/input'
import { executeExtendParse } from '@/lib/internal/extend/operations'
import type { InternalToolOperationHandler } from '@/lib/internal/tool-operations/types'

const logger = createLogger('ExtendToolExecution')
...
export const executeExtendTool: InternalToolOperationHandler = async (request) => {
  request.signal?.throwIfAborted()
  if (!['extend_parser', 'extend_parser_v2'].includes(request.toolId)) {
    return Response.json(
      { success: false, error: `Unsupported Extend tool: ${request.toolId}` },
      { status: 500 }
    )
  }
  ...
  const parsed = extendParseInputSchema.safeParse(request.input)
  if (!parsed.success) return validationResponse(parsed.error)

  try {
    const result = await executeExtendParse(parsed.data, {
    ...
    request.signal?.throwIfAborted()
    if (error instanceof ExtendOperationError) {
      return Response.json(error.body, { status: error.status })
    }
    const message = getErrorMessage(error, 'Internal server error')
    logger.error('Extend operation failed', {
      error: message,
      requestId: request.requestId,
