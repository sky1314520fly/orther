...
import { boxUploadFileInputSchema } from '@/lib/internal/box/schema'
import type { InternalToolOperationHandler } from '@/lib/internal/tool-operations/types'

export const executeBoxTool: InternalToolOperationHandler = async (request) => {
  request.signal?.throwIfAborted()
  const userId = request.context.userId
  if (!userId) {
    return Response.json({ success: false, error: 'Authentication required' }, { status: 401 })
  }
  if (request.toolId !== 'box_upload_file') {
    return Response.json(
      { success: false, error: `Unsupported Box tool: ${request.toolId}` },
      { status: 500 }
    )
  }
  ...
  const parsed = boxUploadFileInputSchema.safeParse(request.input)
  if (!parsed.success) {
    return Response.json(
  ...
  try {
    return await executeBoxUploadFile(parsed.data, {
      userId,
