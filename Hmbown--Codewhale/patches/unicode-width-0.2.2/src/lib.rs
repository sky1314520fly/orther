...
impl UnicodeWidthChar for char {
    #[inline]
    fn width(self) -> Option<usize> {
        // Patched: Enclosed Alphanumerics (circled digits/letters), Dingbat
        // circled digits, and circled numbers on black square have East Asian
        // Width "Ambiguous" but render as 2 columns in CJK terminals. We
        // report 2 here so Ratatui (which calls width() internally) places
        // them in two cells, matching the terminal. Other ambiguous-width
        // characters keep the non-CJK 1-column behavior. (#4479)
        match self {
            '\u{2460}'..='\u{24FF}' | '\u{2776}'..='\u{2793}' | '\u{3248}'..='\u{324F}' => Some(2),
            _ => tables::single_char_width(self),
        }
    }
...
impl UnicodeWidthStr for str {
    #[inline]
    fn width(&self) -> usize {
        // Patched: add 1 extra column for each enclosed-alphanumeric char
        // (unicode-width reports them as 1; terminals render 2). The base
        // non-CJK table keeps every other character's width untouched, so
        // box-drawing glyphs and ellipsis do not regress. (#4479)
