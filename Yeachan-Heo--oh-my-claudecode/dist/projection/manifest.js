 ...
 */
export function normalizeForDigest(content) {
...
export function computeRawDigest(bytes) {
    return createHash('sha256').update(bytes).digest('hex');
}
export function createManifest(engineVersion, sourceRevision, projections) {
...
export function validateManifest(value) {
    ...
    if (typeof m.sourceRevision !== 'string' || !/^[a-f0-9]{64}$/.test(m.sourceRevision))
    ...
    if (!Array.isArray(m.projections))
        ...
        for (let i = 0; i < m.projections.length; i++) {
            ...
            if (typeof r.byteLength !== 'number' || !Number.isSafeInteger(r.byteLength) || r.byteLength < 0)
