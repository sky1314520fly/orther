...
function workerTarget(overrides = {}) {
    return {
        provider: 'tmux',
...
function ownershipDependencies(tmuxOutput = '', cmuxOutputs = []) {
...
describe('direct mailbox target ownership', () => {
    ...
    it('rejects malformed tmux target metadata without executing a provider command', async () => {
        ...
        const result = await verifyTeamTargetOwnership(workerTarget({ providerTarget: 'dispatch-session:workers:extra' }), dependencies);
    ...
    it('rejects provider disagreement without querying either provider', async () => {
        const dependencies = ownershipDependencies('%9\n');
        const result = await verifyTeamTargetOwnership(workerTarget({ provider: 'cmux', providerTarget: 'dispatch-session' }), dependencies);
...
describe('direct mailbox effect adapter', () => {
    function effectDependencies(workerResult, leaderResult = true) {
        return {
            ...
            sendLeader: vi.fn(async () => {
                if (leaderResult instanceof Error)
                    throw leaderResult;
                return leaderResult;
            }),
        };
    ...
    it('does not route an owned cmux surface through tmux when cmux execution context is absent', async () => {
        ...
        try {
            ...
            if (previousSurface === undefined)
                ...
                process.env.CMUX_SURFACE_ID = previousSurface;
        }
    });
...
//# sourceMappingURL=tmux-session.target-membership.test.js.map