...
export async function runRecoverySaga(input, deps) {
    const persistPhase = (value, continuation, adoption, services = 'not_started') => {
    ...
    if (tasks.length === 0)
        persistPhase('reserved', 'none', 'not_started');
    ...
    if (rejected)
        return finalize(failure(input, rejected.error), 'selected', 'not_started');
    ...
    for (const task of tasks) {
        ...
        if (!result.ok)
            return finalize(failure(input, result.error, undefined, Object.keys(sequences).length > 0), 'reserved', 'not_started');
    ...
    if (!pane.ok)
        return finalize(failure(input, pane.error), tasks.length ? 'reserved' : 'none', 'not_started');
    if (!pane.paneId.trim())
        return finalize(failure(input, 'spawn_failed'), tasks.length ? 'reserved' : 'none', 'not_started');
    let persisted;
    if (pane.committed) {
...
//# sourceMappingURL=recovery-saga.js.map