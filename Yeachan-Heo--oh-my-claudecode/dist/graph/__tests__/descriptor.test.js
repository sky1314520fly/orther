...
import { GraphDescriptorValidationError, canonicalJson, computeDescriptorHash, isGraphDescriptor, parseGraphDescriptor, parseSealedGraphDescriptor, sealGraphDescriptor, verifyDescriptorHash, } from "../descriptor.js";
import { graphApprovalDecisionSchema, graphEvidenceReferenceSchema, graphNodeResultSchema, } from "../schema.js";
...
function chainDescriptor(startId, edgeId, terminalId) {
    return {
...
function replaceJoin(descriptor, mutate) {
    return {
        ...descriptor,
        nodes: descriptor.nodes.map((node) => node.kind === "join" ? mutate(node) : node),
    };
}
describe("D1 - unknown fields at any depth rejected (.strict())", () => {
    it("rejects an unknown top-level descriptor field", () => {
        expect(() => parseGraphDescriptor({ ...forkJoinDescriptor(), extra: true })).toThrow(/Unrecognized key/);
    });
    ...
    it("rejects an unknown node-result field", () => {
        expect(() => graphNodeResultSchema.parse({
            outcome: "succeeded",
            attempt_id: "a1",
            evidence_refs: [],
            extra: true,
    ...
    it("rejects an unknown approval-decision field", () => {
        expect(() => graphApprovalDecisionSchema.parse({
            decision: "approved",
            evidence_refs: [],
            extra: true,
        })).toThrow(/Unrecognized key/);
    ...
    it("rejects an unknown evidence-reference field", () => {
        expect(() => graphEvidenceReferenceSchema.parse({
            kind: "file",
            ref: "r1",
            extra: true,
        })).toThrow(/Unrecognized key/);
...
describe("D2 - invalid stable IDs rejected; valid charset accepted", () => {
    it.each([
        ...
        expect(() => parseGraphDescriptor(chainDescriptor(badId, "e1", "b"))).toThrow();
    });
    it("accepts ids using the full stable charset", () => {
        expect(() => parseGraphDescriptor(chainDescriptor("a.b_c:d-1", "e.2", "z9"))).not.toThrow();
    });
});
describe("D3 - duplicate node IDs rejected", () => {
    it("rejects a repeated node id", () => {
        ...
        expect(() => parseGraphDescriptor(input)).toThrow(/duplicate node ID\(s\): a/);
    });
});
describe("D4 - duplicate edge IDs rejected", () => {
    it("rejects a repeated edge id", () => {
        ...
        expect(() => parseGraphDescriptor(input)).toThrow(/duplicate edge ID\(s\): e1/);
    });
});
...
describe("D6 - dangling from/to references rejected", () => {
    ...
    ])("rejects an edge with a %s", (_name, from, to, pattern) => {
        expect(() => parseGraphDescriptor({
            ...chainDescriptor("a", "e1", "b"),
...
describe("D7 - terminal verification rules", () => {
    ...
    it("rejects a non-executable terminal kind (join)", () => {
        ...
        })).toThrow(/terminal verification must be an executable agent or command node/);
    });
...
describe("D9 - cycles: non-back-edge forward cycle rejected; bounded back-edge return accepted", () => {
    it("rejects a forward cycle built from fixed edges", () => {
        ...
        expect(() => parseGraphDescriptor(input)).toThrow(/non-back-edge cycle detected/);
    });
...
describe("D11 - fork/join region rules", () => {
    it("rejects a nested join inside a fork branch", () => {
        ...
        expect(() => parseGraphDescriptor(input)).toThrow(/nested join nested is not allowed inside fork region fan/);
    });
    ...
    it("rejects a join without a matching fan-out node", () => {
        expect(() => parseGraphDescriptor(replaceJoin(forkJoinDescriptor(), (join) => ({
            ...join,
    ...
    it("rejects duplicate branch IDs", () => {
        ...
        expect(() => parseGraphDescriptor({
            ...d,
            edges: d.edges.map((edge) => edge.kind === "fan_out" ? { ...edge, branch_id: "br1" } : edge),
...
describe("D12 - entry-node eligibility", () => {
    ...
    it("rejects an interior fork-branch node as an entry", () => {
        expect(() => parseGraphDescriptor({ ...forkJoinDescriptor(), entry_node_ids: ["b1"] })).toThrow(/entry node b1 must not be inside a fork branch region/);
    });
});
describe("D13 - seal round-trip", () => {
    it("produces a 64-hex hash, verifies, and round-trips through parseSealedGraphDescriptor", () => {
        ...
        expect(sealGraphDescriptor(input).descriptor_hash).toBe(sealed.descriptor_hash);
        expect(parseGraphDescriptor(input)).toEqual(input);
        expect(parseSealedGraphDescriptor(JSON.parse(JSON.stringify(sealed)))).toEqual(sealed);
    });
});
describe("D14 - canonicalJson", () => {
    it("produces compact, recursively key-sorted output with arrays in order", () => {
        ...
        expect(canonicalJson({ outer: { z: [1], a: "x" } })).toBe('{"outer":{"a":"x","z":[1]}}');
    ...
    it("rejects non-plain objects, unsupported primitives, and cycles", () => {
        ...
        expect(() => canonicalJson({ nested: { deep: [cyclicObj] } })).toThrow(TypeError);
    });
});
describe("D15 - hash payload excludes descriptor_hash and runtime fields; tracks content", () => {
    ...
    it("changes when goal, nodes, or edges change", () => {
        const base = forkJoinDescriptor();
        const hash = computeDescriptorHash(base);
        expect(computeDescriptorHash({ ...base, goal: "different goal" })).not.toBe(hash);
        const changedNodes = {
            ...base,
