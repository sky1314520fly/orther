...
export class FileOwnershipFence {
    runsRoot;
    runId;
    staleGraceMs;
    ...
    runDir() {
        if (this.runId === undefined) {
            throw new Error("FileOwnershipFence is not bound to a run; pass runId to the constructor");
        }
    ...
    restoreForeignTombstone(lockPath, tombstone) {
        try {
            ...
            throw new Error(`foreign lock tombstone could not be restored: ${String(error.message ?? error)}`);
        }
    }
