...
import { REGISTRY_SCHEMA_VERSION, REGISTRY_VERSION, WORKFLOW_ENTRIES, } from './registry.js';
// ---------------------------------------------------------------------------
 ...
 */
export function buildRegistryProjection(entries = WORKFLOW_ENTRIES) {
 ...
 */
export function checkProjectionDrift(installed, entries = WORKFLOW_ENTRIES) {
    const registered = new Map(entries.map((e) => [`${e.kind}:${e.name}`, e]));
    const unregistered = [];
