...
function commandExit(script, env) {
    try {
...
describe('project-session-manager worktree result protocol', () => {
    ...
    it.each([
        ...
        if (branch)
            execFileSync(REAL_GIT, ['branch', branch], { cwd: repo });
...
`);
        ...
        const stdout = execFileSync('bash', ['-c', 'source "$SCRIPT_PATH"; psm_get_worktree_root() { printf "%s" "$WORKTREE_ROOT"; }; psm_sanitize() { echo "$1" | tr "[:upper:]" "[:lower:]" | sed "s/[^a-z0-9-]/-/g" | sed "s/--*/-/g" | head -c 30; }; "$CREATOR" "$REPO" "$@"', 'protocol', ...args], {
            encoding: 'utf-8',
            env: {
                ...process.env,
        ...
        if (fields === 3)
            expect(resultFields[2]).not.toBe('');
    });
    it.each([
...
`, {
        ...
        if (malformed)
            expect(execution.stderr).toContain('Malformed worktree result');
    });
...
//# sourceMappingURL=project-session-manager-worktree-protocol.test.js.map