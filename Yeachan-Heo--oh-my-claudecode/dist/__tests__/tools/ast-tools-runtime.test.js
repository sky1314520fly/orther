...
function createRuntime(languages) {
    return {
        Lang: languages,
        parse: parseMock,
    };
...
function expectRecoveryGuidance(text) {
...
describe("ast tool runtime language validation", () => {
    ...
    beforeEach(() => {
        ...
        runtimeLoadMock.mockReturnValue(createRuntime({
    ...
    function createSourceFile(extension, content) {
    ...
    it("reports missing-module recovery guidance and caches the failed load", async () => {
        ...
        expect(firstResult.content[0]?.text).toContain("@ast-grep/napi is not available");
        expectRecoveryGuidance(firstResult.content[0]?.text);
        runtimeLoadMock.mockReturnValue(createRuntime({ TypeScript: "TypeScript" }));
    ...
    it("uses the same missing-module recovery guidance for replace", async () => {
        ...
        expect(result.content[0]?.text).toContain("@ast-grep/napi is not available");
    ...
    it("reports unsupported search language recovery and caches the loaded runtime", async () => {
        ...
        expect(firstResult.content[0]?.text).toContain("Error in AST search: Unsupported language: python");
        expect(firstResult.content[0]?.text).not.toContain("No matches found");
        expectRecoveryGuidance(firstResult.content[0]?.text);
        runtimeLoadMock.mockReturnValue(createRuntime({ TypeScript: "TypeScript", Python: "Python" }));
        const cachedResult = await astGrepSearchTool.handler({
            pattern: "import pandas as pd",
            language: "python",
            path,
    ...
    it("reports unsupported replace language recovery instead of no matches", async () => {
        ...
        expect(result.content[0]?.text).toContain("Error in AST replace: Unsupported language: python");
...
//# sourceMappingURL=ast-tools-runtime.test.js.map