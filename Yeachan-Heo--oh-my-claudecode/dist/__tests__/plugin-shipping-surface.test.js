...
function createFixture(options = {}) {
    ...
    if (options.trackCli !== false)
        git(root, ['add', '-f', '--', 'bridge/cli.cjs']);
    if (options.includeMcpHelper !== false)
        git(root, ['add', '-f', '--', 'bridge/mcp-helper.cjs']);
    if (options.includeCoordinator !== false)
        git(root, ['add', '-f', '--', 'bridge/claude-md-coordinator.cjs']);
...
describe('plugin shipping surface transaction', () => {
    it('fails closed when the declared coordinator is absent from a clean plugin checkout', () => {
        ...
        expect(result.stderr).toContain('required generated runtime file is missing: bridge/claude-md-coordinator.cjs');
    });
    it('fails closed when a reachable generated module is missing', () => {
        const fixture = createFixture({ includeMcpHelper: false });
        const result = run(fixture.root, 'verify');
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain('reachable generated runtime module is missing: bridge/mcp-server.cjs -> ./mcp-helper.cjs');
    });
    ...
    it('discovers helper-computed generated runtime children from local constant strings and arrays', () => {
        const fixture = createFixture();
        writeFileSync(join(fixture.root, 'bridge', 'cli.cjs'), "const root = 'bridge'; const child = ['daemon.js']; const daemon = join(root, ...child); module.exports = require(daemon);\n");
    ...
    it('unwraps safe URL path wrappers around static generated joins', () => {
        const fixture = createFixture();
        writeFileSync(join(fixture.root, 'bridge', 'cli.cjs'), "const child = 'daemon.js'; module.exports = require(pathToFileURL(join('bridge', child)).href);\n");
        writeFileSync(join(fixture.root, 'bridge', 'daemon.js'), 'module.exports = true;\n');
        const result = run(fixture.root, 'verify');
        expect(result.status).toBe(0);
    });
    it('resolves static new URL local runtime loads relative to import metadata', () => {
        const fixture = createFixture();
        writeFileSync(join(fixture.root, 'bridge', 'cli.cjs'), "module.exports = require(new URL('./daemon.cjs', import.meta.url));\n");
    ...
    it('rejects a PR diff with an out-of-closure generated artifact', () => {
        ...
        const result = run(fixture.root, 'check-pr', '--base', base);
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain('pull request changes generated artifacts outside the runtime closure: bridge/unrelated.cjs');
    });
    ...
    it('rejects changes to a base-tracked generated module that is unreachable from plugin entrypoints', () => {
        ...
        const result = run(fixture.root, 'check-pr', '--base', base);
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain('pull request changes generated artifacts outside the runtime closure: dist/unreachable.js');
    });
    ...
    it('does not let a PR-controlled generated directory bless new runtime candidates', () => {
        ...
        const result = run(fixture.root, 'check-pr', '--base', base);
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain('pull request changes generated artifacts outside the runtime closure: dist/unrelated.js');
    });
    ...
    it('rejects ambiguous computed local generated runtime loads', () => {
        const fixture = createFixture();
        writeFileSync(join(fixture.root, 'bridge', 'cli.cjs'), "const name = process.argv[2]; module.exports = require(join('bridge', name));\n");
        const result = run(fixture.root, 'verify');
        expect(result.status).not.toBe(0);
    ...
    it('rejects a required generated runtime artifact that is not tracked at HEAD', () => {
        ...
        const result = run(fixture.root, 'check-pr', '--base', base);
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain('required generated runtime artifacts are not tracked at HEAD: bridge/cli.cjs');
    });
    ...
    it('rejects local check-pr histories without one unique merge base', () => {
        ...
        expect(run(fixture.root, 'check-pr', '--base', orphan).stderr).toContain('check-pr has no common merge base with HEAD');
        ...
        expect(run(fixture.root, 'check-pr', '--base', a2).stderr).toContain('check-pr has ambiguous merge bases: 2');
    });
});
//# sourceMappingURL=plugin-shipping-surface.test.js.map