...
`;
            const out = runShell(`CREATED_FILE=$(mktemp); ${failMock} source "${TMUX_SH}"; psm_create_tmux_session "psm:omc:pr-9" "/tmp" || true`);
    ...
    describe('lookup', () => {
        it('finds a created session whether queried by colon or safe form', () => {
            const out = runShell(`${SOURCE} psm_create_tmux_session "psm:omc:pr-123" "/tmp" >/dev/null;` +
...
`;
    ...
    describe('status reverse lookup', () => {
        it('maps a live tmux-safe session name back to the public id', () => {
            ...
            writeFileSync(join(home, '.psm', 'sessions.json'), JSON.stringify({
                version: 1,
                sessions: {
            ...
            const out = runShell(`source "${CONFIG_SH}"; source "${TMUX_SH}"; source "${SESSION_SH}";` +
                ` psm_get_session_id_for_tmux "psm_omc_issue-42"`, home);
...
//# sourceMappingURL=psm-tmux-naming.test.js.map