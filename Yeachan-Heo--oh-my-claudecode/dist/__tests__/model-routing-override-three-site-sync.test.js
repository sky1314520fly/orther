...
// become depth-aware.
function extractOverrideBlock(source) {
...
describe('MODEL ROUTING OVERRIDE message — three-site sync', () => {
    ...
    it('all three emission sites produce byte-equal override text', async () => {
        ...
        const scriptsLibUrl = pathToFileURL(resolve(__dirname, '../../scripts/lib/model-routing-override-message.mjs')).href;
        ...
        const templateLibUrl = pathToFileURL(resolve(__dirname, '../../templates/hooks/lib/model-routing-override-message.mjs')).href;
        ...
        for (const block of [scriptsSlice, templateSlice, bridgeBlock]) {
            expect(block).toMatch(/ANTHROPIC_DEFAULT_SONNET_MODEL|CLAUDE_CODE_BEDROCK_SONNET_MODEL|OMC_SUBAGENT_MODEL/);
...
//# sourceMappingURL=model-routing-override-three-site-sync.test.js.map