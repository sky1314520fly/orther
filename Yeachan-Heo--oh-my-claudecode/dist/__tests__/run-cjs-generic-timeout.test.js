...
function withWatchdog(promise, timeoutMs = 5000) {
    let timer;
...
describe('run.cjs generic hook timeout supervisor', () => {
    it('exports generic timeout resolution without dispatching when required', () => {
        ...
        expect(winGitInner).toBeGreaterThanOrEqual(runCjs.WINDOWS_GENERIC_STARTUP_MS + runCjs.NESTED_OPERATION_TIMEOUT_MS + runCjs.NESTED_OPERATION_MARGIN_MS);
    ...
    it('reaps the supervised hook tree when its IPC parent disappears', async () => {
        ...
        try {
            ...
            await Promise.race([
                supervisorExit,
                new Promise((_, reject) => setTimeout(() => reject(new Error('supervisor did not exit after IPC disconnect')), 5000)),
            ]);
        }
        finally {
    ...
    it('reaps the detached hook tree when the runner is terminated before its timeout (POSIX)', async () => {
        if (process.platform === 'win32')
            return; // POSIX-only: exercises process-group reap. Killing the grandchild proves its whole group (incl. the direct hook child) was reaped. Windows programmatic SIGTERM force-terminates rather than delivering a catchable signal, so this outer-cancellation path is POSIX-specific.
        ...
        try {
            ...
            grandchildPid = Number(readFileSync(pidfile, 'utf8'));
            expect(grandchildPid).toBeGreaterThan(0);
            const runnerExit = new Promise(resolve => runner.once('exit', () => resolve()));
            runner.kill('SIGTERM');
            await Promise.race([
                runnerExit,
                new Promise((_, reject) => setTimeout(() => reject(new Error('runner did not exit after SIGTERM')), 5000)),
            ]);
            await waitForDeath(grandchildPid);
        }
        finally {
...
`);
        ...
        try {
            ...
            await Promise.race([
                runnerExit,
                new Promise((_, reject) => setTimeout(() => reject(new Error('settling runner ignored SIGTERM')), 3000)),
            ]);
    ...
    it('reaps grandchildren when the runner is cancelled while the hook is an active writer', async () => {
        ...
        try {
            ...
            await Promise.race([
                runnerExit,
                new Promise((_, reject) => setTimeout(() => reject(new Error('runner did not exit after cancel')), 5000)),
            ]);
            await waitForDeath(grandchildPid);
        }
        finally {
...
//# sourceMappingURL=run-cjs-generic-timeout.test.js.map