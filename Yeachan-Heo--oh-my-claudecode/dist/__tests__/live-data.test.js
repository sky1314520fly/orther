...
describe('resolveLiveData - basic', () => {
    it('replaces a basic !command with live-data output', () => {
        ...
        expect(mockedExecFileSync).toHaveBeenCalledWith('echo', ['hello'], expect.objectContaining({ shell: false, timeout: 10_000, windowsHide: true }));
...
describe('resolveLiveData - security', () => {
    ...
    it('parses quoted, escaped, and empty arguments without a shell', () => {
        ...
        expect(mockedExecFileSync).toHaveBeenCalledWith('echo', ['two words', 'literal $HOME', 'escaped space', ''], expect.objectContaining({ shell: false }));
    });
    it('preserves Windows path backslashes in safe arguments', () => {
        mockedExecFileSync.mockReturnValue('ok\n');
        resolveLiveData('!echo C:\\temp\\file.txt "C:\\Program Files\\app.exe" C:\\$Recycle.Bin \\\\server\\share\\file.txt C:\\ "C:\\"');
        expect(mockedExecFileSync).toHaveBeenCalledWith('echo', [
            'C:\\temp\\file.txt',
    ...
    it('keeps quoted shell metacharacters as literal argument data', () => {
        ...
        expect(mockedExecFileSync).toHaveBeenCalledWith('echo', ['; & | < >'], expect.objectContaining({ shell: false }));
    });
});
// ─── Output Parsing ──────────────────────────────────────────────────────────
...
describe('resolveLiveData - multi-line scripts', () => {
    it('executes !begin-script/!end-script blocks', () => {
        ...
        expect(mockedExecFileSync).toHaveBeenCalledWith('bash', [], expect.objectContaining({
            input: 'echo "hello"\necho "world"',
            shell: false,
        }));
        expect(mockedExecSync).not.toHaveBeenCalled();
