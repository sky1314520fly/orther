...
describe('advisory agent final output contract', () => {
    test.each(advisoryAgents)('%s prompt requires substantive final response', (agentName) => {
        ...
        expect(prompt, `${agentName} must explain final message is the Task deliverable`).toMatch(/LAST assistant message is the deliverable surfaced to callers/i);
        expect(prompt, `${agentName} must forbid content-free sign-offs`).toMatch(forbiddenSignoffPattern);
        expect(prompt, `${agentName} must require repeating earlier findings in final response`).toMatch(/repeat the final verdict\/findings structure in the LAST message/i);
    });
...
//# sourceMappingURL=advisory-agent-final-output-contract.test.js.map