...
function readMarkdownDocs() {
    return DOC_ROOTS.flatMap((root) => markdownFiles(join(REPO_ROOT, root))).map((file) => ({
    ...
    it("keeps Claude Code /goal evaluator limitations explicit and evidence-based", () => {
        const violations = claudeGoalDocs().flatMap(({ path, content }) => {
            ...
            return forbiddenClaims
                ...
                .map((pattern) => `${path}: matched forbidden evaluator claim ${pattern}`);
        });
        expect(violations).toEqual([]);
    });
...
//# sourceMappingURL=goal-docs-contract.test.js.map