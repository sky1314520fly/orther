...
import { ClaudeGoalSnapshotError, parseClaudeGoalSnapshot, readClaudeGoalSnapshotInput, reconcileClaudeGoalSnapshot, } from '../claude-goal-snapshot.js';
describe('claude goal snapshot reconciliation', () => {
    ...
    it('detects objective mismatches and incomplete completion proof', () => {
        const mismatch = reconcileClaudeGoalSnapshot(parseClaudeGoalSnapshot({ goal: { objective: 'Different', status: 'active' } }), { expectedObjective: 'Expected', requireSnapshot: true, requireComplete: true });
        expect(mismatch.ok).toBe(false);
        expect(mismatch.errors.join('\n')).toMatch(/objective mismatch/);
        expect(mismatch.errors.join('\n')).toMatch(/not complete/);
    });
    it('accepts compatible complete proof', () => {
        const result = reconcileClaudeGoalSnapshot(parseClaudeGoalSnapshot({ goal: { objective: 'Expected objective', status: 'complete' } }), { expectedObjective: 'Expected objective', requireSnapshot: true, requireComplete: true });
...
//# sourceMappingURL=claude-goal-snapshot.test.js.map