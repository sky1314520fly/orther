 ...
 */
export function parseClaudeGoalSnapshot(value) {
    ...
    const objective = safeString(goal.objective
        ?? goal.condition
        ?? goal.goal
        ?? goal.description
        ?? root.objective
    ...
    const tokenBudget = safeNumber(goal.token_budget
        ?? goal.tokenBudget
        ?? root.token_budget
        ?? root.tokenBudget);
...
export async function readClaudeGoalSnapshotInput(raw, cwd = process.cwd()) {
    if (!raw?.trim())
        return null;
...
export function reconcileClaudeGoalSnapshot(snapshot, options) {
...
export function formatClaudeGoalReconciliation(reconciliation) {
    const parts = [...reconciliation.errors, ...reconciliation.warnings];
    return parts.join(' ');
}
//# sourceMappingURL=claude-goal-snapshot.js.map