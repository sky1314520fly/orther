...
vi.mock("../dispatcher.js", () => ({
    dispatchNotifications: (...args) => mockDispatchNotifications(...args),
    sendDiscord: vi.fn(),
...
vi.mock("../config.js", () => ({
    ...
    getTmuxTailLines: () => 15,
    isEventAllowedByVerbosity: (verbosity, event) => mockIsEventAllowedByVerbosity(verbosity, event),
...
vi.mock("../hook-config.js", () => ({
    ...
    mergeHookConfigIntoNotificationConfig: (_hook, config) => config,
}));
...
describe("explicit event notification verbosity", () => {
    ...
    it("keeps default session verbosity as a gate for ask-user-question when the event is not explicitly enabled", async () => {
        ...
        expect(mockIsEventAllowedByVerbosity).toHaveBeenCalledWith("session", "ask-user-question");
    ...
    it("dispatches explicitly enabled ask-user-question even when default session verbosity would otherwise block it", async () => {
        ...
        expect(mockDispatchNotifications).toHaveBeenCalledWith(expect.objectContaining({
            events: { "ask-user-question": { enabled: true } },
        }), "ask-user-question", expect.objectContaining({
...
//# sourceMappingURL=explicit-event-verbosity.test.js.map