...
const repoFile = (relative) => readFileSync(fileURLToPath(new URL(`../../../../${relative}`, import.meta.url)), 'utf-8');
...
const LABEL_EXAMPLES_FRAGMENT = (() => {
    ...
    if (!description)
        throw new Error('executionLabel lost its description');
...
describe('committed shipping-surface parity (#3682)', () => {
    it('.mcp.json loads the gated bridge server', () => {
        const mcpConfig = JSON.parse(repoFile('.mcp.json'));
    ...
    for (const artifact of PYTHON_REPL_ARTIFACTS) {
        it(`${artifact} ships the current executionLabel examples`, () => {
            expect(repoFile(artifact), `${artifact} is stale: rebuild it`).toContain(LABEL_EXAMPLES_FRAGMENT);
        });
    ...
    it('dist/agents/scientist.js ships sandbox-truthful agent metadata', () => {
        ...
        for (const phrase of ['ml/hypothesis', 'pandas', 'numpy', 'matplotlib', 'scipy']) {
            expect(content, `dist/agents/scientist.js must not contain "${phrase}"`).not.toContain(phrase);
        }
    });
...
//# sourceMappingURL=shipped-artifact-parity.test.js.map