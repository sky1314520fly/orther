...
import { toSdkToolFormat } from '../../index.js';
...
describe('python_repl sandbox guidance parity (#3682)', () => {
    ...
    describe('scientist agent guidance', () => {
        ...
        it('does not claim standard deviation needs a blocked library', () => {
            ...
            expect(prompt).not.toMatch(/standard deviations?[^.]{0,120}(require|blocked|unavailable|cannot)/i);
            expect(prompt).not.toMatch(/(require|blocked|unavailable|cannot)[^.]{0,120}standard deviations?/i);
        });
    });
    describe('serialized inputSchema surfaces', () => {
        ...
        const schemas = [
            ...
            [
                ...
                toSdkToolFormat(pythonReplToolIndex).inputSchema,
            ],
        ];
        for (const [name, schema] of schemas) {
            it(`${name} serves parameter descriptions, so schema text is user-facing guidance`, () => {
                const label = schema.properties.executionLabel;
    ...
    describe('repo documentation surfaces', () => {
        const readDoc = (relative) => readFileSync(fileURLToPath(new URL(relative, import.meta.url)), 'utf-8');
...
describe('extractPythonGuidance', () => {
    ...
    it('flags a blocked-library claim inside the relevant section', () => {
        const polluted = doc.replace('Sandboxed: imports are blocked.', 'Sandboxed: use pandas dataframes for analysis.');
        expect(extractPythonGuidance(polluted).toLowerCase()).toContain('pandas');
    });
});
//# sourceMappingURL=sandbox-guidance-parity.test.js.map