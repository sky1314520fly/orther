...
import { claimSessionEndAction, claimSessionEndJob, finishSessionEndAction, isManifestTerminal, markSessionEndActionRunner, mutateSessionEndJob, prepareCoreManifest, readSessionEndJob, reapStaleSessionEndOwner, releaseSessionEndJob, renewSessionEndLease, sealCoreManifest, sealWikiManifest, sessionEndJobsDirectory, takeSessionEndDiscoveryPage, } from '../cleanup-manifest.js';
const directories = [];
function project() {
...
function preparedAndSealed(directory, sessionId = 'session-a') {
...
describe('durable SessionEnd cleanup manifest', () => {
    ...
    it('renews leases and reaps only expired, positively dead or PID-reused owners', () => {
        ...
        const reap = reapStaleSessionEndOwner;
        ...
        const expiresAgain = JSON.parse(readFileSync(path, 'utf8'));
    ...
    it('persists action claim, runner arm, result, and terminality separately from discovery tickets', () => {
        ...
        for (const [name] of Object.entries(current.actions)) {
            ...
            current = mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
                job.actions[name].status = 'completed';
                job.actions[name].runner = { attempt: 1, runnerNonce: `${name}-runner`, phase: 'terminal', deadlineAt: new Date().toISOString() };
            });
        }
    ...
    it('keeps more than sixteen queued jobs discoverable through bounded rotating pages', () => {
        ...
        for (let page = 0; page < 6; page++)
            for (const sessionId of takeSessionEndDiscoveryPage(directory, 4))
    ...
    it('bounds required failures and never retries a failed remote delivery attempt', () => {
        ...
        for (const name of ['callback', 'notification', 'openclaw']) {
    ...
    it('does not requeue a best-effort action after its owner dies with delivery uncertain', () => {
        ...
        const expired = JSON.parse(readFileSync(manifestPath, 'utf8'));
        expired.owner.leaseExpiresAt = new Date(Date.now() - 1_000).toISOString();
...
//# sourceMappingURL=cleanup-manifest.test.js.map