...
describe('hook registry — derivation from installed hooks.json (#3707)', () => {
    ...
    it('derives risk class by convention: only pre-tool-enforcer and permission-handler fail closed', () => {
        ...
        expect(failClosedEntrypoints).toEqual(new Set(['pre-tool-enforcer.mjs', 'permission-handler.mjs']));
    });
...
describe('hook registry — selection and ordering', () => {
    ...
    it('parses installed commands into entrypoint + args', () => {
        expect(parseEntrypointCommand('node "$CLAUDE_PLUGIN_ROOT"/scripts/run.cjs "$CLAUDE_PLUGIN_ROOT"/scripts/subagent-tracker.mjs start')).toEqual({ entrypoint: 'subagent-tracker.mjs', args: ['start'] });
