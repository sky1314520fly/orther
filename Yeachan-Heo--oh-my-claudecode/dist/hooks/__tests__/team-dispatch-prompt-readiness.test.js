...
async function seedMismatchedTarget() {
    await writeFile(join(teamDir, 'config.json'), JSON.stringify({
        tmux_session: 'dispatch-session',
        workers: [
...
async function seedIncompleteCursorTarget() {
    await seed('cursor');
...
async function runDefaultInjector() {
    const result = await drainPendingTeamDispatch({ cwd: root, stateDir, logsDir, maxPerTick: 1 });
    const requests = JSON.parse(await readFile(join(teamDir, 'dispatch', 'requests.json'), 'utf8'));
    return { result, request: requests[0] };
}
describe('team dispatch provider-aware prompt readiness', () => {
    beforeEach(async () => {
        tmuxUtilsMocks.tmuxExecAsync.mockReset().mockImplementation(async (args) => {
...
//# sourceMappingURL=team-dispatch-prompt-readiness.test.js.map