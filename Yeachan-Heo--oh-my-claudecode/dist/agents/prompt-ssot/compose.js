...
function overlaySectionId(overlay) {
    const ids = [];
    if (overlay.provider)
        ids.push(`provider/${overlay.provider}`);
    if (overlay.modelTier)
        ids.push(`tier/${overlay.modelTier}`);
    return ids;
}
export function selectSections(manifest, sections, projectionId, overlay = {}) {
    const spec = manifest.projections.find((p) => p.id === projectionId);
    if (!spec)
        throw new PromptSsotError(`unknown projection: ${projectionId}`);
    const byId = new Map(sections.map((s) => [s.id, s]));
    const wanted = [...spec.sections];
    if (spec.acceptsOverlays)
        wanted.push(...overlaySectionId(overlay));
    const selected = [];
    for (const id of wanted) {
        const section = byId.get(id);
        if (!section)
            throw new PromptSsotError(`projection ${projectionId} references missing section: ${id}`);
        selected.push(section);
    }
    for (const required of manifest.requiredSections) {
        if (!selected.some((s) => s.id === required)) {
            throw new PromptSsotError(`projection ${projectionId} is missing required section: ${required}`);
        }
    }
...
export function composeProjection(manifest, sections, projectionId, overlay = {}) {
...
export function composeAll(manifest, sections, overlay = {}) {
