...
export function buildClosureReport(
  receipts: AliasRetirementReceipt[],
  options: { cwd?: string; exists?: (p: string) => boolean } = {},
): ClosureReport {
  const cwd = options.cwd ?? process.cwd();
  const exists = options.exists ?? ((p: string) => existsSync(join(cwd, p)));

  const byAlias = new Map(receipts.map((r) => [r.alias.toLowerCase(), r]));

  const entries = receipts.flatMap((r) =>
    r.generatedArtifacts.map((p) => {
      ...
      return {
        ...
        deletableOnlyAfterEligible: true as const,
        exists: exists(p),
        deletableNow,
