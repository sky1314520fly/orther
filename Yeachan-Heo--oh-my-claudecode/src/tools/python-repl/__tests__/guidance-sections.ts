...
interface Unit {
  /** Lines belonging to this unit (a heading line, a plain line, or a fenced block). */
  lines: string[];
  /** ATX heading level when the unit is a heading, otherwise undefined. */
  headingLevel?: number;
}
...
function toUnits(markdown: string): Unit[] {
 ...
 */
export function extractPythonGuidance(markdown: string): string {
  ...
  for (const unit of units) {
    ...
    if (unit.headingLevel !== undefined) {
      ...
      if (openLevel !== null) kept.push(text);
      continue;
    }
    if (openLevel !== null || RELEVANT.test(text)) kept.push(text);
  }
