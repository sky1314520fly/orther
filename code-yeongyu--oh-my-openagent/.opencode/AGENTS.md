...
- `skills/work-with-pr-workspace/` iteration + eval subdirs (`evals/`, `iteration-1/eval-*/`) — benchmark data, not runtime code: paired `with_skill`/`without_skill` grading + timing JSON and generated output Markdown; `iteration-1/review.html` is a self-contained viewer (inline `APP_DATA`, localStorage feedback, JSON download). Regenerated by skill evals, never hand-edited.
